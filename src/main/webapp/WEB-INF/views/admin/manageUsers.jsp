...
<head>
  <meta charset="UTF-8">
  <title>Manage Users - QueueSmart</title>
  <style>
...
<div class="main">
  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 30px;">
    <div>
        <h2 style="font-size: 36px; font-weight: 800; margin-bottom: 10px;">User Management</h2>
        <p style="color: var(--text2); font-size: 16px;">Approve, suspend, or remove users from the platform.</p>
    </div>
  ...
  <c:if test="${not empty param.error}">
    <div class="alert alert-error">Error processing action. Please try again.</div>
  </c:if>
      ...
      <tbody>
        <c:forEach var="user" items="${users}">
          <tr>
            ...
            <td><span style="background:var(--inp); padding:4px 8px; border-radius:6px; font-size:12px; font-weight:800; border:1px solid var(--bdr);">${user.role}</span></td>
            ...
            <td>
              <c:if test="${user.role != 'ADMIN'}">
                <form action="${pageContext.request.contextPath}/admin/users" method="POST" style="display:inline;">
                  ...
                  <button type="submit" name="action" value="delete" class="btn-sm btn-err" onclick="return confirm('Are you sure you want to permanently delete this user?')">Delete</button>
                </form>
              </c:if>
            </td>
          </tr>
